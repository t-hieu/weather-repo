...
class LWPerformanceDetailViewController4: LWBaseViewController , LWViewBase35Delegate, TRPickerViewDelegate,  LWLiftMaterialRowviewDelegate, LWChoseCustomerViewDelegate, LWBasicInformationViewDelegate, LWLiftMaterialSelectViewControllerDelegate5, LWViewBaseDelegate, LWPeopleRubbishRowDelegate, LWViewOneLabelDelegate, TRDatePickerViewDelegate,MainStoryboard, LWChoseConstructionViewDelegate, KeyboardDelegate, UITextFieldDelegate, LWChoseFloorDelegate, LWChoseManyFloorDelegate{
...
//    var NameOfTypeisSelected:String?
    ...
    let Equipment = 1
    let Architect = 2
    var liftUsage: LWLiftUsageModel4?
    var liftUsageTemp: LWLiftUsageModel4?
    var liftUsageShow: LWLiftUsageModel4?
...
//
        ...
        if LWUserDefaults.getConstructionId().isEmpty, let vc = ConstructionSelectViewController.createInstanceFromStoryboard() as? ConstructionSelectViewController{
            ...
            if(isFirstLoad){
                ...
                if(self.stateTabbar != LW_TABBAR_PERFORMANCE_CONFIRM){
                    self.liftUsage = LWLiftUsageModel4.init()
    ...
    @IBAction func tapConfirmButton(_ sender: Any) {
        ...
        switch self.stateController {
        case LW_PERFORMANCE_INPUT_STATUS_SHOW?:
            self.liftUsageShow = LWLiftUsageModel4.init()
    ...
    func updateData() {
        ...
        if let count = self.liftUsage?.expandGoods?.count{
            for i in 0..<count{
                goodNames += COMMA_MARK + (self.liftUsage?.expandGoods![i].goodName)!
            }
        }
        ...
        AlamofireManager.shared.request(APIRouter.get(url: API.LW_URL_LIFT_USAGE_DETAIL, params: params, identifier: nil)) { (response) in
            ...
            if response != nil{
                guard let liftUsageData = response!["liftUsage"] as? [String:Any],
                    let liftUsage:LWLiftUsageModel4 = Mapper<LWLiftUsageModel4>().map(JSON: liftUsageData),
    ...
    func tapLiftMaterialCellDelegate() {
        ...
        if((self.stateController == LW_PERFORMANCE_INPUT_STATUS_MODIFY || stateController == LW_PERFORMANCE_INPUT_STATUS_CREATE_NEW) && self.liftUsage?.goodType == LW_PEOPLE_RUBBISH_STATE_GOOD){
            ...
            if(self.liftUsage?.customer == nil || (self.liftUsage?.customer?.customerId == nil) || self.liftUsage?.customer?.customerId == 0){
                ...
                let choseGoodViewController = self.storyboard?.instantiateViewController(withIdentifier: "LWLiftMaterialSelectViewController4") as! LWLiftMaterialSelectViewController4
...
//        self.TypeisSelected = TypeisSelected
    ...
    //Hieu
   func liftMaterialSelectTap4(liftMaterials: [LWGoodModel], unitQuantity: String, NameOfTypeisSelected: String) {
       var goodNames: String = ""
       let count = liftMaterials.count
       for i in 0..<count{
           if i != count - 1{
               goodNames += liftMaterials[i].goodName as! String + "-"
           }else{
               goodNames += liftMaterials[i].goodName as! String
           }
      ...
      if count > 1{
        self.liftUsage?.expandGoods = (liftMaterials[1..<count] as! [LWGoodModel])
      }
