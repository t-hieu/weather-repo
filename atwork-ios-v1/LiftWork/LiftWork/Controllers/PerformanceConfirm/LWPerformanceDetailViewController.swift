...
class LWPerformanceDetailViewController: LWBaseViewController , LWViewBase35Delegate, TRPickerViewDelegate,  LWLiftMaterialRowviewDelegate, LWChoseCustomerViewDelegate, LWBasicInformationViewDelegate, LWLiftMaterialSelectViewControllerDelegate, LWViewBaseDelegate, LWPeopleRubbishRowDelegate, LWViewOneLabelDelegate, TRDatePickerViewDelegate,MainStoryboard, LWChoseConstructionViewDelegate, KeyboardDelegate, UITextFieldDelegate, LWChoseFloorDelegate, LWChoseManyFloorDelegate{
...
//    var NameOfTypeisSelected:String?
    ...
    let Equipment = 1
    let Architect = 2
    var liftUsage: LWLiftUsageModel?
    var liftUsageTemp: LWLiftUsageModel?
    var liftUsageShow: LWLiftUsageModel?
...
//
        ...
        if LWUserDefaults.getConstructionId().isEmpty, let vc = ConstructionSelectViewController.createInstanceFromStoryboard() as? ConstructionSelectViewController{
            ...
            if(isFirstLoad){
                ...
                if(self.stateTabbar != LW_TABBAR_PERFORMANCE_CONFIRM){
                    self.liftUsage = LWLiftUsageModel.init()
    ...
    @IBAction func tapConfirmButton(_ sender: Any) {
        ...
        switch self.stateController {
        case LW_PERFORMANCE_INPUT_STATUS_SHOW?:
            self.liftUsageShow = LWLiftUsageModel.init()
    ...
    func updateData() {
        ...
        var goodNames: String! = self.liftUsage?.good?.goodName
        if let count = self.liftUsage?.expandGoods?.count{
            for i in 0..<count{
                goodNames += MATERIAL_MARK + (self.liftUsage?.expandGoods![i].goodName)!
            }
        }
        self.liftMaterialRowView.contentLabel.text = goodNames
        //End
        ...
        AlamofireManager.shared.request(APIRouter.get(url: API.LW_URL_LIFT_USAGE_DETAIL, params: params, identifier: nil)) { (response) in
            ...
            if response != nil{
                guard let liftUsageData = response!["liftUsage"] as? [String:Any],
                    let liftUsage:LWLiftUsageModel = Mapper<LWLiftUsageModel>().map(JSON: liftUsageData),
...
//                self.roomListValues.reverse()
    ...
    func updatePerformance(){
        ...
        var goodIdExpands: String! = ""
        if let count = liftUsage?.expandGoods?.count{
            for i in 0..<count{
            
                if i != count - 1{
                    goodIdExpands += (liftUsage?.expandGoods![i].goodId?.description)! + COMMA_MARK
                }else{
                    goodIdExpands += (liftUsage?.expandGoods![i].goodId?.description)!
                }
                
            }
        }
        
        params["goodIdExpands"] = goodIdExpands
        
        //params["expandGoods"] = self.liftUsage?.expandGoods
        //End
    ...
    func CheckPerformance(){
        ...
        if let count = liftUsage?.expandGoods?.count{
            for i in 0..<count{
                    ...
                    if i != count - 1{
                        goodIdExpands += liftUsage?.expandGoods![i].goodId?.description as! String + COMMA_MARK
                    }else{
                        goodIdExpands += liftUsage?.expandGoods![i].goodId?.description as! String
                    }
    ...
    func tapLiftMaterialCellDelegate() {
        ...
        if((self.stateController == LW_PERFORMANCE_INPUT_STATUS_MODIFY || stateController == LW_PERFORMANCE_INPUT_STATUS_CREATE_NEW) && self.liftUsage?.goodType == LW_PEOPLE_RUBBISH_STATE_GOOD){
            ...
            if(self.liftUsage?.customer == nil || (self.liftUsage?.customer?.customerId == nil) || self.liftUsage?.customer?.customerId == 0){
                ...
                let choseGoodViewController = self.storyboard?.instantiateViewController(withIdentifier: "LWLiftMaterialSelectViewController") as! LWLiftMaterialSelectViewController
...
//        self.TypeisSelected = TypeisSelected
   ...
   func liftMaterialSelectTapMany(liftMaterials: [LWGoodModel], unitQuantity: String, NameOfTypeisSelected: String) {
       ...
       let count = liftMaterials.count
       for i in 0..<count{
           if i != count - 1{
               goodNames += liftMaterials[i].goodName! + MATERIAL_MARK
           }else{
               goodNames += liftMaterials[i].goodName!
           }
           
       }
    
       self.liftMaterialRowView.contentLabel.text = goodNames
       //self.liftUsage?.goods = liftMaterials

      self.liftUsage?.good = liftMaterials[0]
      if count > 1{
        self.liftUsage?.expandGoods = Array(liftMaterials[1..<count])
      }
    ...
    func updateStatePeopleRubbish(state: String) {
        //hieu
        self.liftUsage?.good = nil
        self.liftUsage?.expandGoods = nil
        //End
