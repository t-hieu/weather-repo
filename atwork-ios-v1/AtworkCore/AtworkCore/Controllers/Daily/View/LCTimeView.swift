...
public class LCTimeView: UIView {
    ...
    func initSubview() {
        ...
        let nib = UINib(nibName: "LCTimeView", bundle: bundle)
...
//        Bundle.main.loadNibNamed("LCTimeView", owner: self, options: nil)
        ...
        for index in 0..<48 {
            var titleLabel = ""
            if index % 2 == 0 {
                titleLabel = String(index/2 + 6)
                titleLabel += ":00"
            }
            if index % 2 == 0 && (index/2 + 6)<10 {
                titleLabel = "0"+String(index/2 + 6)
                titleLabel += ":00"
            }
            
            let timeView = self.viewTimeForSize(frame: CGRect.init(x: 0, y: CGFloat(index) * SCHEDULE_HEIGHT/2, width: self.contentView.frame.size.width, height: SCHEDULE_HEIGHT/2), timeLabel: titleLabel)
