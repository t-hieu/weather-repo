...
//        self.tableView.register(UINib.init(nibName: "FromToViewCellLeft", bundle: Bundle.init(for: FromToViewCell.self)), forCellReuseIdentifier: "FromToViewCellLeft")
        ...
        self.schedule = ATScheduleModel.init()
...
//            self.view.activityIndicatorView.stopAnimating()
            if response != nil{
                self.view.activityIndicatorView.stopAnimating()
                guard let schedule : ATScheduleModel = Mapper<ATScheduleModel>().map(JSONObject: response!["schedule"] ),
...
//                self.vehicles = vehicles
                self.lifts = lifts
                //Hieu
                self.allowChangeSchdule = response!["allowChange"] as! Bool
                //End
                self.updateData()
...
//            }
            ...
            if !isCustomerApp {
                if schedule.createUserModel != nil && schedule.createUserModel.customerId == 1 {
                    ...
                    if self.allowChangeSchdule{
                        self.modeEditStatus = MODE_EDIT_SCHEDULE
                    }else{
                        self.modeEditStatus = MODE_EDIT_PRO_CUS
                    }
                    //End
...
//                        self.navigationController?.popViewController(animated: true)
    ...
    public func isEditRow(tagRow: Int) -> Bool{
        ...
        case TAG_ROW_ELV:
            return true
        case TAG_ROW_CATEGORY, TAG_ROW_GOOD, TAG_ROW_GATE, TAG_ROW_FROM, TAG_ROW_TO, TAG_ROW_TIME, TAG_ROW_VERHICLE, TAG_ROW_PACKING, TAG_ROW_IS_FORK_LIFT, TAG_ROW_NOTE:
...
//                }
                ...
                cell?.updateData(titleText: "揚重元-先", contentTextFrom: self.schedule.departure ?? "", contentTextTo: self.schedule.arrival ?? "", textAlignment: .center, tagView: TAG_ROW_FROM, isActive: self.isEditRow(tagRow: TAG_ROW_FROM), delegate: self, isForce: !isEventMode)
                return cell!
